...
export default function Topbar() {
  ...
  async function loadProfileName(fallbackName = "User") {
    try {
      ...
      setUserName(profile.name?.trim() || fallbackName);
    } catch {
      setUserName(fallbackName);
    }
  }

  function getMetadataName(user: any) {
  ...
  function saveSeenNotificationIds(ids: string[]) {
    ...
    setSeenNotificationIds(uniqueIds);
    localStorage.setItem(SEEN_NOTIFICATIONS_KEY, JSON.stringify(uniqueIds));
    window.dispatchEvent(new CustomEvent("notifications-seen", { detail: uniqueIds }));
  }
  ...
  useEffect(() => {
    setMounted(true);
    setSeenNotificationIds(getStoredSeenNotificationIds());

    async function getUser() {
      const { data: { session } } = await supabase.auth.getSession();
      if (session?.user) {
        loadProfileName(getMetadataName(session.user));
        setIsLoggedIn(true);
        loadNotifications();
      } else {
        setUserName("Guest");
    ...
    function handleNotificationsSeen(event: Event) {
      ...
      if (Array.isArray(customEvent.detail)) {
        setSeenNotificationIds(customEvent.detail);
      }
    }

    window.addEventListener("profile-updated", handleProfileUpdated);
    window.addEventListener("notifications-seen", handleNotificationsSeen);

    return () => {
      subscription.unsubscribe();
      window.removeEventListener("profile-updated", handleProfileUpdated);
      window.removeEventListener("notifications-seen", handleNotificationsSeen);
    };
  }, []);
    ...
    mounted && notificationsOpen
      ? createPortal(
          <div
            ...
            className="fixed left-3 right-3 top-24 z-[9999] max-h-[70vh] overflow-hidden rounded-[22px] border border-white/90 bg-white p-3 shadow-2xl shadow-cyan-900/20 sm:left-auto sm:right-8 sm:top-28 sm:w-96"
          >
